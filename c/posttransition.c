...
void send_all_acceptable_packages(town* source, int source_office_index, town* target, int target_office_index) {
    ...
    while (i<source_office->packages_count) {
        if (source_office->packages[i].weight>=target_office->min_weight
            ...
            // move it to target office
            target_office->packages = (package*)realloc(
                target_office->packages, ++target_office->packages_count*sizeof(package));
            ...
            source_office->packages = (package*)realloc(
                source_office->packages, --source_office->packages_count*sizeof(package));
        } else {
            i++;
        }
    }
...
int pakages_count(town t) {
    int npkgs = 0;
    for (int i=0; i<t.offices_count; i++) {
        npkgs += t.offices[i].packages_count;
    }
    return npkgs;
}
...
town town_with_most_packages(town* towns, int towns_count) {
    int k=0;
    int maxpkgs = pakages_count(towns[k]);
    for (int i=1; i<towns_count; i++) {
        int pkgcount = pakages_count(towns[i]);
        if (maxpkgs<pkgcount) {
            k = i;
            maxpkgs = pkgcount;
        }
